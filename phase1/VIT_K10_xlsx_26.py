...
def print_layer(conv_model, layers_params, model_txt):
    """
    ...
    nm_model = f"{layers_params['id_test']}_{model_txt}"
    print(nm_model)
...
# In[3]:
def tl_vit_b16(num_classes, IMAGE_SIZE=224):
...
def tl_vit_b32(num_classes, IMAGE_SIZE=224):
...
def hyper_model(config_model):
    ...
    if model == 'VIT_B16':
        ...
        vit_model = tl_vit_b16(num_classes, IMAGE_SIZE=img_size)
        vit_model.summary()
    
    if model == 'VIT_B32':
        print('num_classes: ',num_classes)
        vit_model = tl_vit_b32(num_classes, IMAGE_SIZE=img_size)
        vit_model.summary()
    ...
    model.add(Dense(config_model['dense_size'], activation=config_model['activation']))
    model.add(BatchNormalization())
    model.add(Dense(num_classes, activation=config_model['last_activation']))
    ...
    print('depth ', depth)
    layers_params={'id_test':id_test, 'save_dir':save_dir, 'model': model}
    print_layer(vit_model, layers_params, config_model['model'])
    
    model.summary()
    nm_summary=save_dir+str(id_test)+'_'+config_model['model']+'.txt'
    with open(nm_summary, 'w') as f:
...
if __name__ == "__main__":
    ...
    default_path_sheet = "./results/phase1/ViT_26/0_F1_VIT.xlsx"
